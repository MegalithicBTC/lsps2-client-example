...
fn main() {
	...
	let desc =
		Bolt11InvoiceDescription::Direct(Description::new("test-invoice-5-sats".into()).unwrap());
	let invoice = node.bolt11_payment().receive(5_000, &desc, 3_600).unwrap();

	println!("INVOICE for 5 satoshis:\n{invoice}\n");
