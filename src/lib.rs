...
pub fn setup_node(storage_dir: String, log_path: String) -> Arc<ldk_node::Node> {
	...
	let lsp_pubkey_str =
		env::var("LSP_PUBKEY").expect("LSP_PUBKEY must be set in .env or environment");
	let lsp_pubkey = PublicKey::from_str(&lsp_pubkey_str).expect("Invalid LSP_PUBKEY format");
	...
	let esplora_url =
		env::var("ESPLORA_API_URL").expect("ESPLORA_API_URL must be set in .env or environment");
	...
	let lsp_address =
		env::var("LSP_ADDRESS").expect("LSP_ADDRESS must be set in .env or environment");
