...
fn main() {
	...
	// ── create invoice ─────────────────────────────────────────────────────
	let desc =
		Bolt11InvoiceDescription::Direct(Description::new("test-megalith-lsps2".into()).unwrap());
	let invoice =
		node.bolt11_payment().receive_via_jit_channel(5_000_000, &desc, 3_600, None).unwrap();
